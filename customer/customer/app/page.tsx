        ...
        <main className="flex-1 p-4 lg:p-6 space-y-8 max-w-full overflow-x-hidden">
              ...
              <div className="max-w-2xl">
                <div className="flex items-center gap-2 text-white/90 mb-2">
                  ...
                  <span className="text-sm font-medium">
                    {user.isLoggedIn ? `Welcome back, ${user.name.split(' ')[0]}!` : 'Welcome to NexaShop!'}
                  </span>
                </div>
          ...
          <section>
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
              {vendors.map((vendor) => (
                <Link key={vendor.name} href={`/search?q=${encodeURIComponent(vendor.name)}`}>
          ...
          <section>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
              {allProducts.slice(12, 20).map((product) => (
                <ProductCard key={product.id} product={product} />
              ))}
            </div>
          </section>
          ...
          <section>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
              {allProducts.slice(20, 28).map((product) => (
                <ProductCard key={product.id} product={product} />
              ))}
            </div>
          </section>
              ...
              <div>
                ...
                <div className="space-y-2">
                  {["Electronics", "Fashion", "Home & Living", "Sports"].map(l => (
                    <Link key={l} href={`/search?q=${encodeURIComponent(l)}`} className="block text-sm text-muted-foreground hover:text-foreground transition-colors">{l}</Link>
                  ))}
                </div>
              </div>
              <div>
